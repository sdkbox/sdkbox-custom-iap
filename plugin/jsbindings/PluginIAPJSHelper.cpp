...
void product_to_obj(JSContext* cx, const sdkbox::Product& p, JS::MutableHandleObject retObj)
{
    ...
    const char16_t *chars = (const char16_t*)cocos2d::cc_utf8_to_utf16(p.price.c_str());
...
public:
    ...
    void setProductIds(const std::vector<std::string>& products) {
    ...
    void callback(float dt) {
        ...
        if (0 == _eventName.compare("onInitialized")) {
            ...
            dataVal[1] = SB_STR_TO_JSVAL(cx, _msg);
            datalen = 2;
        } else if (0 == _eventName.compare("onShouldAddStorePayment")) {
            dataVal[0] = SB_STR_TO_JSVAL(cx, _msg);
            datalen = 1;
        } else if (0 == _eventName.compare("onFetchStorePromotionOrder")) {
            JS::RootedValue jsret(cx);
            sdkbox::std_vector_string_to_jsval(cx, _productIds, &jsret);
            dataVal[0] = jsret.get();
            dataVal[1] = SB_STR_TO_JSVAL(cx, _error);
            datalen = 2;
        } else if (0 == _eventName.compare("onFetchStorePromotionVisibility")) {
            dataVal[0] = SB_STR_TO_JSVAL(cx, _msg);
            dataVal[1] = JS::BooleanValue(_bValue);
            dataVal[2] = SB_STR_TO_JSVAL(cx, _error);
            datalen = 3;
        } else if (0 == _eventName.compare("onUpdateStorePromotionOrder")) {
            dataVal[0] = SB_STR_TO_JSVAL(cx, _error);
            datalen = 1;
        } else if (0 == _eventName.compare("onUpdateStorePromotionVisibility")) {
            dataVal[0] = SB_STR_TO_JSVAL(cx, _error);
            datalen = 1;
        } else if (0 == _eventName.compare("onPurchaseHistory")) {
            dataVal[0] = SB_STR_TO_JSVAL(cx, _msg);
            datalen = 1;
        } else {
            return;
...
private:
    ...
    std::vector<std::string> _productIds;
    JSObject* _jsHandler;
...
//        JsIAPCallbackObj* obj = JsIAPCallbackObj::create("onShouldAddStorePayment", getJSDelegate());
//        obj->setMsg(productId);
//        obj->start();

        JSContext* cx = s_cx;
        const char* func_name = "onShouldAddStorePayment";

        JS::RootedObject obj(cx, proFunc->get());
        JSAutoCompartment ac(cx, obj);
        ...
        dataVal[0] = SB_STR_TO_JSVAL(cx, productId);
        datalen = 1;
        ...
        bool retBool;
        if (sdkbox::js_to_bool(cx, retval, &retBool)) {
            return retBool;
        } else {
            return false;
        }
    ...
    void onFetchStorePromotionOrder(const std::vector<std::string>& productIds, const std::string& error) {
        JsIAPCallbackObj* obj = JsIAPCallbackObj::create("onFetchStorePromotionOrder", getJSDelegate());
    ...
    void onFetchStorePromotionVisibility(const std::string productId, bool visibility, const std::string& error) {
        JsIAPCallbackObj* obj = JsIAPCallbackObj::create("onFetchStorePromotionVisibility", getJSDelegate());
        obj->setBoolean(visibility);
        obj->setMsg(productId);
        obj->setError(error);
        obj->start();
    }
    ...
    void onUpdateStorePromotionOrder(const std::string& error) {
        JsIAPCallbackObj* obj = JsIAPCallbackObj::create("onUpdateStorePromotionOrder", getJSDelegate());
        obj->setError(error);
        obj->start();
    }

    void onUpdateStorePromotionVisibility(const std::string& error) {
        JsIAPCallbackObj* obj = JsIAPCallbackObj::create("onUpdateStorePromotionVisibility", getJSDelegate());
        obj->setError(error);
        obj->start();
    }

    void onPurchaseHistory(const std::string& purchases) {
        JsIAPCallbackObj* obj = JsIAPCallbackObj::create("onPurchaseHistory", getJSDelegate());
        obj->setMsg(purchases);
        obj->start();
    }
